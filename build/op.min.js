Array.prototype.indexOf||(Array.prototype.indexOf=function(h){if(null==this)throw new TypeError;var a=Object(this),g=a.length>>>0;if(0===g)return-1;var b=0;1<arguments.length&&(b=Number(arguments[1]),b!=b?b=0:0!=b&&Infinity!=b&&-Infinity!=b&&(b=(0<b||-1)*Math.floor(Math.abs(b))));if(b>=g)return-1;for(b=0<=b?b:Math.max(g-Math.abs(b),0);b<g;b++)if(b in a&&a[b]===h)return b;return-1});
...
var ObjectPath=function(a,h){this.exprCache=[];this._init_(a,h);return this};
...
"array"].indexOf(typeof a)?(this.log(a+" is not object nor array! Data not changed."),a):this.data=a},setDebug:function(a){this.D=a},flatten:function(a){var h=[],l=function(a){if(Array.isArray(a))for(var g=0;g<a.length;g++)l(a[g]);else if("object"===typeof a)for(g in h.push(a),a)l(a[g])};l(a);return h},execute:function(a){var h=this,l=this.flatten,m=this.simpleTypes,g,f=function(b){if(Array.isArray(b)){for(var c=[],a=0;a<b.length;a++)c.push(f(b[a]));return c}var e=b.id;if(0<="+;-;*;%;/;>;>=;<;<=;in;not in;is;is not".split(";").indexOf(b.id))var c=
"object"===typeof b.first&&b.first.id?f(b.first):null,d="object"===typeof b.second&&b.second.id?f(b.second):null,g=typeof c,k=typeof d;switch(e){case "(literal)":return b.value;case "+":if("number"===g&&"number"!==k)d=parseInt(d);else if(Array.isArray(c)){if(Array.isArray(d))return c.concat(d);c.push(d)}return c+d;case "-":return d?c-d:-c;case "*":return"wildcard"!=b.arity?c*d:null;case "%":return c%d;case "/":return c/d;case ">":return c>d;case "<":return c<d;case ">=":return c>=d;case "<=":return c<=
d;case "not":return!f(b.first);case "or":return f(b.first)||f(b.second);case "and":return f(b.first)&&f(b.second);case "in":case "not in":return a=null,"string"===k?a=0<=d.search(c.toString()):Array.isArray(d)?a=0<=d.indexOf(c):"object"===k&&(a=d.hasOwnProperty(c)),"in"===e?a:!a;case "is":case "is not":a=null;if(0<=m.indexOf(g))a=c==d;else if("array"===g||"object"===g)try{throw a=JSON.stringify(c)==JSON.stringify(d),{name:"comparison Error",message:"JSONStringifyNotAvailable. Your web browser doesn't support JSON.stringify(). Vote for support at"};
}catch(n){}return"is"===e?a:!a;case "(root)":return h.data;case "(current)":return h.current;case "(context)":return h.context;case ":":return b;case "(name)":return b.value;case "[":if("unary"===b.arity){e=[];for(a in b.first)e.push(f(b.first[a]));return e}if("op"===b.arity){c=f(b.first);if(!c)return c;if(Array.isArray(c)){d=f(b.second);k=typeof d;if(d&&":"===d.id)return c.slice(f(d.first),f(d.second));if("number"===k)return-1===d?c.slice(-1)[0]:0>d?c.slice(d,d+1)[0]:c[d];if("string"===k){e=[];for(a=
0;a<c.length;a++)c[a][d]&&e.push(c[a][d]);return e}if("object"===typeof b.second&&0<=h.SELECTOR_OPS.indexOf(b.second.id)){b=b.second;e=[];b=Object.create(b);for(a in c)d=c[a],h.current=d,b.first=d,f(b)&&e.push(d);return e}programmingError("left is array and right is not number")}else if("object"===g&&("(name)"===b.second.id||"string"===k))return c[d];return 1}return null;case "(":switch(b.first.value){case "str":return a=f(b.second),"object"===typeof a?JSON.stringify(a):a.toString}return null;case "{":case "":throw{error:"NotImplementedYet",
message:e+" is not implemented yet!",data:b};case "..":c=l(f(b.first));case ".":c=c||f(b.first);if("*"===b.second.id)return c;if(c){if(Array.isArray(c)){e=[];d=f(b.second);for(a=0;a<c.length;a++)c[a][d]&&e.push(c[a][d]);return e}return c[b.second.value]}return null;case "fn":switch(b.first){case "replace":return e=f(b.second),e[0]?e[0].replace(new RegExp(e[1],"g"),e[2]):"";case "split":return e=f(b.second),e[0]?e[0].split(e[1]):"";case "max":return Math.max.apply(null,f(b.second)[0]);case "min":return Math.min.apply(null,
f(b.second)[0])}throw{error:"WrongFunction",message:"Function "+e+" is not proper ObjectPath function.",data:b};}throw{error:"WrongOperator",message:"Operator "+e+" is not proper ObjectPath operator.",data:b};};if(!a)return a;"string"===typeof a&&(g=this.compile(a));return f(g)}};
